...
from MyDataset import vim1_fmri_transpose_dataset
import itertools
...
def vim1_create_normlized_zq(dt_key):
    ...
    with h5py.File(
        ...
        with h5py.File("/data1/home/guangjie/Data/vim-2-gallant/myOrig/imagenet128_embeds_from_vqvae.hdf5", 'r') as ef:
            embeds = ef['embeds'][:]
            with h5py.File(
                    "/data1/home/guangjie/Data/vim2/extract_from_vqvae/zq_of_{}_fmap_mm_scaler_23x23_blur3.hdf3".format(
                        dt_key), 'w') as zqf:
                latent = zqf.create_dataset('latent', shape=(len(k), 23, 23, 128))
                for i, _k in enumerate(k):
                    ...
                    for j in range(128):
                        ...
                        small_fm = cv2.resize(fm, (23, 23))
                        blur_small_fm = cv2.GaussianBlur(small_fm, (3, 3), 0)
                        latent[i, :, :, j] = blur_small_fm
                        # norm_blur_fm = np.clip((blur_fm - np.min(blur_fm)) / np.max(blur_fm - np.min(blur_fm)), 0, 1)
                        # grid = np.concatenate((norm_fm, norm_blur_fm), axis=1)
                        #
                        # cv2.imshow('img', blur_small_fm)
                        # cv2.waitKey(1)
                        # print('img')
...
if __name__ == '__main__':
    ...
    # select_valid_voxel(rois=[7], subject=1)
    # print(len(idx))
