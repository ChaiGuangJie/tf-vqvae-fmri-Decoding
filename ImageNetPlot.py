...
with h5py.File("/data1/home/guangjie/Data/vim-2-gallant/myOrig/k_from_vqvae_sv.hdf5", 'w') as kf:
    kData = kf.create_dataset('k', shape=(len(dataset), 32, 32), dtype=np.uint16)
    begin_idx = 0
    for step, data in enumerate(dataLoader):
        end_idx = begin_idx + data.shape[0]
        k = sess.run(net.k, feed_dict={x: data})
        kData[begin_idx:end_idx] = k
        begin_idx = end_idx
...
# with h5py.File("/data1/home/guangjie/Data/vim-2-gallant/myOrig/k_from_vqvae_sv.hdf5", 'w') as kf:
#     # embeds = sess.run(net.embeds)
#     # zef.create_dataset('embeds', data=embeds)
#     # print(embeds)
#     # recData = sf.create_dataset('rec', shape=(len(dataset), 128, 128, 3), dtype=np.uint8)
#     # zeData = zef.create_dataset('ze', shape=(len(dataset), 32, 32, 128))  # z_e.shape = (?, 32, 32, 128)
#     kData = kf.create_dataset('k', shape=(len(dataset), 32, 32))
#     with h5py.File("/data1/home/guangjie/Data/vim-2-gallant/myOrig/zq_from_vqvae_sv.hdf5", 'w') as zqf:
#         zqData = zef.create_dataset('zq', shape=(len(dataset), 32, 32, 128))  # z_q.shape = (?, 32, 32, 128)
#
#         begin_idx = 0
#         for step, data in enumerate(dataLoader):
#             end_idx = begin_idx + data.shape[0]
#
#             ze, zq = sess.run((net.z_e, net.z_q), feed_dict={x: data})
#             # rec = sess.run(net.p_x_z, feed_dict={x: data})
#             # recData[begin_idx:end_idx] = (rec * 255.0).astype(np.uint8)
#             zeData[begin_idx:end_idx] = ze
#             zqData[begin_idx:end_idx] = zq
#             begin_idx = end_idx
#
#             if step % 100 == 0:
#                 print(step)
